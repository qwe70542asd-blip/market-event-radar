...
class StaticTests(unittest.TestCase):
 ...
 def test_version(self):
  self.assertEqual(json.loads(self.read("VERSION.json"))["baseline_version"],"11.4.29")
 ...
 def test_service_worker_cache(self):
  sw=self.read("service-worker.js");self.assertIn("market-event-radar-v11-4-29",sw)
 ...
 def test_market_snapshot_seed_schema(self):
  ...
  self.assertEqual(payload.get("metadata",{}).get("version"),"v11.4.29")
 ...
 def test_all_pages_current_version(self):
  for p in ROOT.glob("*.html"):
   ...
   self.assertIn("v11.4.29",body,p.name)
 ...
 def test_balanced_portfolio_summary_layout(self):
  ...
  for token in ("v11.4.29 portfolio summary balance","grid-template-columns:repeat(3,minmax(0,1fr))","grid-template-rows:repeat(2,minmax(78px,1fr))","justify-content:center"):
 ...
 def test_v11424_version_bump_prevents_same_version_asset_cache(self):
  for path in ("index.html","assets/shared.js","assets/home.js","assets/sw-register.js","service-worker.js","VERSION.json"):
   content=self.read(path);self.assertIn("11.4.29",content);self.assertNotIn("11.4.23",content);self.assertNotIn("11.4.22",content)
 ...
 def test_v11426_15m_is_documented_as_post_deploy_nonblocking(self):
  audit=self.read("docs/V11.4.29-release-audit.md")
 ...
 def test_v11428_mobile_calendar_has_no_forced_horizontal_scroll(self):
  ...
  for token in (".calendar-weekdays,.calendar-grid{width:100%;min-width:0!important","overflow:visible!important","grid-template-columns:repeat(7,minmax(0,1fr))",".mobile-install-trigger","assets/pwa-install.js?v=11.4.29"):
