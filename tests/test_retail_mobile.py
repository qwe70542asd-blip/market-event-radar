...
def test_all_app_pages_load_shared_mobile_release_css():
    for name in ('index.html','asset.html','portfolio.html','tw-market.html','institutional.html','news.html','event.html','data-status.html'):
        text=read(name);assert 'assets/v11.4.54-overrides.css?v=11.4.54' in text,name;assert 'assets/shared.js?v=11.4.54' in text,name
def test_phone_quick_navigation_is_fixed_above_bottom_nav():
    css=read('assets/v11.4.54-overrides.css');assert '.mobile-global-quick-nav{display:none}' in css;assert 'position:fixed!important' in css;assert 'bottom:calc(62px + env(safe-area-inset-bottom))!important' in css;assert 'grid-template-columns:repeat(4,minmax(0,1fr))!important' in css;assert '.shell,.asset-page{padding-bottom:calc(128px + env(safe-area-inset-bottom))!important}' in css
def test_non_home_pages_receive_same_four_quick_actions():
    shared=read('assets/shared.js')
    for token in ('index.html#calendarPanel','tw-market.html','institutional.html','index.html#latestNews'):assert token in shared
    assert 'installGlobalMobileQuickNav' in shared and 'floating-quick-nav mobile-global-quick-nav' in shared
