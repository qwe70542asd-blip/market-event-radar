...
def test_all_app_pages_load_shared_mobile_release_css():
    for name in ('index.html','asset.html','portfolio.html','tw-market.html','institutional.html','news.html','event.html','data-status.html'):
        text=read(name);assert 'assets/v11.4.58-overrides.css?v=11.4.58' in text,name;assert 'assets/shared.js?v=11.4.58' in text,name
def test_phone_uses_bottom_navigation_only_and_does_not_cover_content():
    css=read('assets/v11.4.58-overrides.css');html=read('index.html')
