...
- 7 個 HTML 頁面的 ID 唯一性與本機檔案引用檢查。
- GitHub Actions Workflow 規則檢查：5 分鐘 `frequent`、首次 `all`、新聞／公告不得忽略錯誤、空資料停止發布。
- 4 項最後交易日單元測試：跨時區日線、禁止舊日覆蓋、同日修正、ETF 成交值排行。
- 5 項新聞／公告單元測試：20 天回補查詢、20 天保留邊界、同標題合併來源、官方日期解析、官方原文優先。
- `live-data` 共用路徑測試：行情、新聞、法人與標的明細皆會移除前面的 `data/`，對應分支根目錄。
- 首頁 ID 唯一性與區塊順序檢查：ETF 行情之後為虛擬貨幣，近期高影響事件位於下層。
- 第一次上傳的 Workflow 模式已確認為 `all`。
- 首頁月曆標頭、法人上市／上櫃切換、官方公告清單與 10 則最新新聞的結構檢查。
