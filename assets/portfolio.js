...
function renderRetailCalculator(){const price=finite($("#retailToolPrice")?.value),qty=Math.floor(finite($("#retailToolQty")?.value)||0),budget=finite($("#retailToolBudget")?.value),type=$("#retailToolType")?.value||"stock",taxRate=type==="etf"?.001:.003;const gross=price!=null&&qty>0?price*qty:null,buy=gross==null?null:gross*(1+standardBrokerFeeRate),sell=gross==null?null:gross*(1-standardBrokerFeeRate-taxRate),breakEven=((1+standardBrokerFeeRate)/(1-standardBrokerFeeRate-taxRate)-1)*100,maxQty=price!=null&&price>0&&budget!=null&&budget>0?Math.floor(budget/(price*(1+standardBrokerFeeRate))):null;if($("#retailBuyAmount"))$("#retailBuyAmount").textContent=buy==null?"—":`NT$ ${fmt(buy,0)}`;if($("#retailSellNet"))$("#retailSellNet").textContent=sell==null?"—":`NT$ ${fmt(sell,0)}`;if($("#retailBreakEven"))$("#retailBreakEven").textContent=`${fmt(breakEven,2)}%`;if($("#retailMaxQty"))$("#retailMaxQty").textContent=maxQty==null?"—":`${fmt(maxQty,0)} 股`}
function renderWatchlist(){const root=$("#watchlistRows");if(!root)return;const list=loadWatchlist();root.innerHTML=list.map(item=>{const symbol=String(item.symbol).toUpperCase(),q=quotes.get(symbol),candidate=candidateMap.get(symbol)||{},price=finite(q?.price),change=finite(q?.change_percent),assetClass=item.asset_class||candidate.asset_class||q?.asset_class||"stock",lot=price==null?null:price*1000*(1+standardBrokerFeeRate);return `<article class="watchlist-card"><a href="asset.html?symbol=${encodeURIComponent(symbol)}"><small>${escapeHtml(assetClass==="etf"?"ETF":"股票")}</small><strong>${escapeHtml(symbol)} ${escapeHtml(item.name||candidate.name||q?.name||"")}</strong><span>${price==null?"等待行情":`NT$ ${fmt(price,2)}`} <em class="${cls(change)}">${change==null?"":pct(change)}</em></span><b>${lot==null?"1 張金額待行情":`1 張約 NT$ ${fmt(lot,0)}`}</b></a><div><button class="btn" type="button" data-watch-calc="${escapeHtml(symbol)}">試算</button><button class="btn" type="button" data-watch-del="${escapeHtml(symbol)}">移除</button></div></article>`}).join("")||'<div class="empty">尚未加入自選標的；到個股／ETF 詳情頁按「加入自選」。</div>';root.querySelectorAll("[data-watch-del]").forEach(button=>button.addEventListener("click",()=>{const symbol=button.dataset.watchDel;saveWatchlist(loadWatchlist().filter(item=>String(item.symbol).toUpperCase()!==symbol));renderWatchlist()}));root.querySelectorAll("[data-watch-calc]").forEach(button=>button.addEventListener("click",()=>{const symbol=button.dataset.watchCalc,q=quotes.get(symbol),candidate=candidateMap.get(symbol)||{},price=finite(q?.price);if(price!=null)$("#retailToolPrice").value=String(price);$("#retailToolType").value=(candidate.asset_class||q?.asset_class)==="etf"?"etf":"stock";renderRetailCalculator();$("#retailTools")?.scrollIntoView({behavior:"smooth",block:"start"})}))}
...
render();
loadData("home-assets.json",assets).then(fresh=>{if(Array.isArray(fresh?.assets)&&fresh.assets.length){assets=fresh;rebuildMarketData();render()}}).catch(()=>{});
loadData("tw-market-compact.json",tw).then(fresh=>{if(Array.isArray(fresh?.items)&&fresh.items.length){tw=fresh;rebuildMarketData();render()}}).catch(()=>{});
