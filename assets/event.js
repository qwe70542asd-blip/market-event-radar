(async()=>{
  "use strict";
  const {$,escapeHtml,formatTime,formatEventTime,loadData,stripHtml,renderNewsThumb,safeExternalHref}=MR;
  ...
  function renderEvent(){
    ...
    $("#eventDetail").innerHTML=`<section class="event-hero"><p class="eyebrow">${escapeHtml(event.region||"GLOBAL")} · ${escapeHtml(event.event_type||event.category||"event")}</p><h1>${escapeHtml(event.title)}</h1></section><section class="event-meta-grid"><article class="event-meta-card"><small>${event.all_day===true||event.time_status==="date-only"?"事件日期":"事件時間"}</small><strong>${escapeHtml(formatEventTime(event))}</strong></article><article class="event-meta-card"><small>影響程度</small><strong class="impact-text ${escapeHtml(event.impact||"medium")}">${escapeHtml(impactLabel(event.impact))}</strong></article><article class="event-meta-card"><small>公布確認</small><strong>${event.announced_at?escapeHtml(formatTime(event.announced_at)):"既有官方日期"}</strong></article><article class="event-meta-card"><small>日期狀態</small><strong>${escapeHtml(dateStatus)}</strong></article></section>${facts.length?`<section class="event-content-card"><h2>關鍵資訊</h2><div class="event-facts-grid">${facts.slice(0,12).map(row=>`<article class="event-fact-card"><small>${escapeHtml(row.label)}</small><strong>${escapeHtml(row.value)}</strong></article>`).join("")}</div></section>`:""}<section class="event-content-card"><h2>事件說明</h2><div class="event-description-list">${paragraphs.slice(0,12).map(text=>`<p>${escapeHtml(text)}${/[。！？]$/.test(text)?"":"。"}</p>`).join("")}</div></section>${event.market_effect?`<section class="event-content-card"><h2>可能影響</h2><p>${escapeHtml(stripHtml(event.market_effect))}</p></section>`:""}${event.previous_start?`<p class="notice">原日期：${escapeHtml(formatTime(event.previous_start,{dateOnly:event.all_day===true||event.time_status==="date-only"}))}</p>`:""}<section class="event-content-card"><h2>資料來源</h2><p>${escapeHtml(event.source_name||"官方公告")}</p><div class="event-source-actions">${sourceActions}</div>${rawDescription?`<details class="event-raw-details"><summary>查看原始公告文字</summary><pre>${escapeHtml(rawDescription)}</pre></details>`:""}</section><div id="eventRelatedMount"></div>`;
    renderRelated();
  }
