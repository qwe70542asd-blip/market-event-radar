...
(()=>{
  ...
  const assessDates=(marketDate,chipDate,referenceDate)=>{
    ...
    // Calendar-age fallback is used only when the verified ^TWII session is unavailable.
  ...
  // Exported only for deterministic regression tests; no user data is exposed.
  ...
  function applyMarketStale(){
    if(applying||!state?.marketStale)return;applying=true;
    ...
    document.getElementById("marketStateSummary")?.classList.remove("chip-data-stale");queueMicrotask(()=>{applying=false});
  }
  function applyChipStale(){
    ...
    // Keep valid price/breadth/volume conclusions. Only institutional-flow conclusions are closed.
    setText("foreignDirection","待更新","flat");
    setText("foreignDirectionDetail",`法人 ${dayText(chipDate)||"未確認"}／行情 ${dayText(marketDate)||"未確認"}`);
    setText("briefForeign","待更新");
    document.getElementById("marketStateSummary")?.classList.remove("market-data-stale");
    document.getElementById("marketStateSummary")?.classList.add("chip-data-stale");queueMicrotask(()=>{applying=false});
  }
  function clearGuard(){const root=document.getElementById("marketStateSummary");root?.classList.remove("market-data-stale","chip-data-stale")}
  async function assess(){
    ...
    try{
      ...
      state=assessDates(marketDate,chipDate,referenceDate);
      if(state.marketStale)applyMarketStale();else if(state.chipsStale)applyChipStale();else clearGuard();
    }catch(error){console.warn("stale market guard assessment failed",error)}
  }
  const boot=()=>{assess();setTimeout(assess,2000);setInterval(assess,60000);const root=document.getElementById("marketStateSummary");if(root)new MutationObserver(()=>{if(applying)return;if(state?.marketStale)applyMarketStale();else if(state?.chipsStale)applyChipStale()}).observe(root,{subtree:true,childList:true,characterData:true,attributes:true})};
