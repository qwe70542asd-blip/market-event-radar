(() => {
  ...
  function render() {
    ...
    $("#newsPageGrid").innerHTML = items.map(item => {
      ...
      const label = window.MarketNewsLink?.linkLabel?.(item) || (mode === "direct" ? "閱讀原文" : "來源首頁");
      return `
  ...
  if (initialQuery && $("#newsSearchInput")) $("#newsSearchInput").value = initialQuery;
  syncAssetFilters();
