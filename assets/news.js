(async()=>{
  ...
  const majorScore=item=>{const text=`${item.title||""} ${item.summary||""}`;let score=0;if(SYSTEMIC_RE.test(text))score+=42;if(ASIA_RISK_RE.test(text))score+=18;if(ASIA_RISK_RE.test(text)&&ASIA_STRESS_RE.test(text))score+=26;if(ASIA_RISK_RE.test(text)&&ASIA_CROSS_BORDER_RE.test(text))score+=10;if(LEADER_RE.test(text))score+=24;if(LEADING_SECTOR_RE.test(text))score+=18;if(EXECUTIVE_RE.test(text))score+=16;if(BUSINESS_RE.test(text))score+=14;if(["official-notices","company-disclosures","cna"].includes(item.source_id))score+=12;if((item.other_reports||[]).length)score+=Math.min(14,(item.other_reports||[]).length*5);if(item.impact==="high")score+=12;else if(item.impact==="medium")score+=5;const age=(Date.now()-Date.parse(item.published_at||item.date||0))/86400000;if(Number.isFinite(age)&&age<=1)score+=10;else if(Number.isFinite(age)&&age<=3)score+=6;return Math.max(score,Number(item.importance_score||0))};
  ...
  const card=item=>{const company=companyLabel(item),image=renderNewsThumb(item,"tile",{alt:item.title});return `<a class="portal-news-card${image?"":" no-image"}" href="${escapeHtml(item.url)}" target="_blank" rel="noreferrer noopener">${image}<div class="portal-news-body"><div class="portal-card-meta"><span>${escapeHtml(item.ai_category||item.topic||"財經新聞")}</span><time>${escapeHtml(formatTime(item.published_at||item.date))}</time></div>${company?`<b class="stock-chip">${escapeHtml(company)}</b>`:""}<h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(truncate(item.summary,92)||"查看完整新聞內容與最新進展。")}</p><div class="portal-card-foot"><span class="impact-badge ${escapeHtml(item.impact||"medium")}">${impactLabel(item)}</span><span>${escapeHtml(item.market_direction||"中性")}</span></div></div></a>`};
