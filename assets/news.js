(async()=>{
  ...
  const noticeCard=item=>`<article class="company-notice-card"><div class="company-notice-head"><div><span class="tag">${escapeHtml(item.ai_category||"個股公告")}</span><span class="impact-badge ${escapeHtml(item.impact||"medium")}">${impactLabel(item)}</span></div><time>${escapeHtml(formatTime(item.published_at||item.date))}</time></div><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(truncate(item.summary,210)||"來源未提供公告摘要。")}</p>${Array.isArray(item.key_facts)&&item.key_facts.length?`<dl class="notice-facts">${item.key_facts.slice(0,6).map(row=>`<div><dt>${escapeHtml(row.label||"重點")}</dt><dd>${escapeHtml(row.value||"—")}</dd></div>`).join("")}</dl>`:""}<div class="company-notice-actions">${(item.symbols||[])[0]?`<a href="asset.html?symbol=${encodeURIComponent(item.symbols[0])}">查看個股 →</a>`:""}<a href="${escapeHtml(item.url)}" target="_blank" rel="noreferrer noopener">官方來源 →</a></div></article>`;
