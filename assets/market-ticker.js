(() => {
  ...
  function formatTradeDate(item) {
    const value = item?.trading_date || item?.as_of || "";
    const match = String(value).match(/^(\d{4})-(\d{2})-(\d{2})/);
    return match ? `${Number(match[2])}/${Number(match[3])}` : "等待交易日";
  }
  ...
  function indexRow(item) {
    ...
    return `<a class="market-index-row ${direction(item)}${stale}" href="${escapeHtml(item.link || item.source_url || '#')}" target="_blank" rel="noreferrer noopener"
      title="${escapeHtml(`${item.source || "等待來源"}｜交易日 ${formatTradeDate(item)}｜${item.delay || ""}`)}">
  ...
  function etfRow(item, index) {
    ...
    return `<a class="market-etf-row ${direction(item)}" href="${escapeHtml(href)}" target="_blank" rel="noreferrer noopener"
      title="${escapeHtml(`${item.source || "行情來源"}｜交易日 ${formatTradeDate(item)}｜${item.delay || ""}`)}">
  ...
  function updateStatus(payload) {
    ...
    if (status) {
      status.textContent = healthy
        ? `${healthy} 項｜各市場最後交易日｜更新 ${formatTime(metadata.updated_at) || "等待更新"}（台灣）｜每 30 秒檢查`
        : "行情等待第一次排程";
