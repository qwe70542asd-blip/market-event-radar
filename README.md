...
4. 解壓 `market-event-radar-v11.4.54-full-replace.zip`，把 ZIP **裡面的所有內容**直接放到 repository root。不要多包一層資料夾。
5. 確認 `index.html`、`.github`、`assets`、`data`、`scripts` 都直接位於 repository root。
6. 執行一次 `CLEAN-REPO.cmd`。正常應顯示 `repository layout clean for v11.4.54` 或完成清理。
7. GitHub Desktop Commit：`market-event-radar-v11.4.54-full-replace`，再 Push。
8. GitHub Actions 至少確認：
   - `Verify v11.4.54 stable app release` 綠燈
   - `Update v11.4.54 event calendar and announced dates` 綠燈
   - `Deploy v11.4.54 live market worker` 綠燈
