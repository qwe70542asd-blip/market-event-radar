...
- 移除會顯示「This symbol is only available on TradingView」的外部 ticker。
- 建立 TWSE、TPEx、Yahoo、Stooq、FRED 與 CoinGecko 多來源資料管線。
- 收錄台美日韓指數、VIX、美債殖利率、匯率、原油、黃金及主要虛擬貨幣。
...
- 台股指數改為加權在上、櫃買在下；日韓指數改為日經在上、KOSPI 在下。
- 美股四大指數維持 2×2，三個指數區塊等高，移除原本側欄大面積空白。
- 行情板最大寬度縮至 1280px，字體與數字同步放大。
- 台股與美股 ETF 改為一次顯示 4 檔向上輪播，代碼、名稱、價格與漲跌分欄顯示。

## v10.6.0 首頁完整整合版

- 首頁資訊順序改為：分類行情、重要新聞、個人投資／幣圈、法人／公告、事件月曆、今日財經快訊、資料健康。
- 事件搜尋、地區、類型、高影響及產業焦點直接併入月曆，不再獨立占用大區塊。
- 月曆是首頁主要內容，右側近期事件限制為四筆緊湊摘要。
- 今日財經快訊只呈現真正的文章標題、發布時間、分類、來源與直接文章網址。
- 無法確認文章原址的內容不在首頁顯示，不再導向 Google 搜尋或來源首頁冒充新聞。
- 加入公開媒體、官方公告、國際新聞及台灣券商／投顧公開內容的分類來源。
- 虛擬貨幣區輪播市值、交易量、穩定幣、短線爆量與最新代幣五組排行。
- 所有排程資料改寫入獨立 `live-data` 分支，網站程式保留在 `main`，避免 GitHub Desktop 再與機器人資料更新發生合併衝突。
- 網頁優先讀取 `live-data` 最新 JSON，失敗時回退到安裝包內的本機資料與 seed。
