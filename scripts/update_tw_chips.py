...
def parse_tpex_day_trade_market(rows: list[dict[str, Any]], fallback_date: str | None) -> tuple[dict[str, Any], str | None]:
    ...
    The parser is schema-first: it recognises the current official OpenAPI fields
...
def main() -> None:
    ...
    payload = {
        "metadata": {
            ...
            "note": "官方資料優先；第三方只補缺漏。ROC/Gregorian 日期、TPEx 市場彙總當沖欄位與線上 schema gate，不偽造個股當沖資料。",
        },
        "markets": markets,
        "items": items,
