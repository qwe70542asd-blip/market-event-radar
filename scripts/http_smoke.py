...
REQUIRED = {
    "index.html": ["v11.4.49", "今日台股狀態", "六大指數互動 K 線與關鍵資訊"],
...
def get(path: str) -> tuple[int, str, str]:
    ...
    request = Request(url, headers={"User-Agent": "MarketEventRadarReleaseSmoke/11.4.49"})
...
def main() -> None:
    ...
    for page in PAGES:
        ...
        assert ("v11.4.49" in body or "v11.4.46" in body), page
    ...
    expected_data_versions = {
        "data/market-snapshot-seed.js": "v11.4.46",
        "data/market-kline-seed.js": "v11.4.46",
        "data/events-seed.js": "v11.4.49",
        "data/stock-basics-seed.js": "v11.4.46",
    }
    for path, version in expected_data_versions.items():
        status, _, body = get(path)
        assert status == 200 and version in body, (path, version)
