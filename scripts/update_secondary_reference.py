...
VERSION="v11.4.15"
HEADERS={"User-Agent":"Mozilla/5.0 (compatible; MarketEventRadar/11.4.15)"}
BATCH=80
...
def fetch_one(asset:dict)->tuple[str,dict|None,str|None]:
 ...
 try:
  ...
  valid=[number(v) for v in closes if number(v) is not None]
  price=number(meta.get("regularMarketPrice")) or (valid[-1] if valid else None)
  previous=number(meta.get("chartPreviousClose")) or number(meta.get("previousClose")) or (valid[-2] if len(valid)>1 else None)
  ...
  return symbol,{
   ...
   "market_time":meta.get("regularMarketTime"),"source":"Yahoo Finance chart",
...
def main()->None:
 ...
 payload={
  "metadata":{"version":VERSION,"updated_at":NOW.isoformat(timespec="seconds"),"status":"ok" if success else "partial" if items else "warning","item_count":len(items),"batch_size":len(batch),"batch_success":success,"note":"Yahoo chart is a secondary quote reference only; official TWSE/TPEx values remain primary."},
