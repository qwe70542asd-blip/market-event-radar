#!/usr/bin/env python3
"""Reachability-aware TPEx live contract verification for v11.4.39.
...
* unavailable: network/HTTP/JSON failure -> warning, release continues;
...
def fetch_json(session: requests.Session, url: str, *, attempts: int = 3, timeout: int = 20) -> FetchResult:
...
def main() -> None:
    ...
    checks = (check_exdiv, check_dividend_plans, check_day_trade)
    warnings: list[str] = []
    failures: list[str] = []
    for check in checks:
        try:
