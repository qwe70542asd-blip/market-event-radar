/**
 * Market Event Radar v11.4.53 edge live-market service.
...
const baseHeaders={"access-control-allow-origin":"*","access-control-allow-methods":"GET,OPTIONS","access-control-allow-headers":"content-type","cache-control":"no-store","x-content-type-options":"nosniff","referrer-policy":"no-referrer"};
...
function localParts(date,market){
  const parts=new Intl.DateTimeFormat("en-CA",{timeZone:TZ[market],weekday:"short",year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hour12:false}).formatToParts(date);
  return Object.fromEntries(parts.map(item=>[item.type,item.value]));
}
function marketOpen(market,date=new Date()){
  const p=localParts(date,market);
  if(["Sat","Sun"].includes(p.weekday))return false;
  const minutes=Number(p.hour)*60+Number(p.minute);
  return SESSIONS[market].some(([start,end])=>minutes>=start-5&&minutes<=end+5);
}
...
function chartRows(chart){
  const quote=chart?.indicators?.quote?.[0]||{},times=chart?.timestamp||[],rows=[];
  for(let index=0;index<Math.min(times.length,(quote.open||[]).length,(quote.high||[]).length,(quote.low||[]).length,(quote.close||[]).length);index++){
    const open=num(quote.open[index]),high=num(quote.high[index]),low=num(quote.low[index]),close=num(quote.close[index]);
    if([open,high,low,close].some(value=>value===null))continue;
    rows.push({time:Number(times[index]),open,high,low,close,volume:num((quote.volume||[])[index])});
  }
  return rows;
}
function aggregate4h(rows,market){
  const groups=new Map();
  for(const row of rows){
    const p=localParts(new Date(row.time*1000),market),minutes=Number(p.hour)*60+Number(p.minute),sessions=SESSIONS[market]||[];
    let session=-1,start=0;
    for(let i=0;i<sessions.length;i++){if(minutes>=sessions[i][0]&&minutes<=sessions[i][1]){session=i;start=sessions[i][0];break}}
    if(session<0)continue;
    const bucket=Math.floor((minutes-start)/240),key=`${p.year}-${p.month}-${p.day}:${session}:${bucket}`,group=groups.get(key)||[];
    group.push(row);groups.set(key,group);
  }
  const out=[];
  for(const group of groups.values()){
    group.sort((a,b)=>a.time-b.time);
    out.push({time:group[0].time,open:group[0].open,high:Math.max(...group.map(row=>row.high)),low:Math.min(...group.map(row=>row.low)),close:group.at(-1).close,volume:group.reduce((sum,row)=>sum+(row.volume||0),0)});
  }
  return out.sort((a,b)=>a.time-b.time);
}
async function yahoo(symbol,range="3mo",interval="1d"){
  if(!ALLOWED_SYMBOLS.has(symbol))throw new Error("symbol not allowed");
  ...
  const response=await fetch(url,{headers:{"user-agent":"Mozilla/5.0 MarketEventRadar/11.4.53","accept-language":"zh-TW,en;q=.7"}});
  if(!response.ok)throw new Error(`${symbol} Yahoo ${response.status}`);
  const body=await response.json(),chart=body?.chart?.result?.[0];
  if(!chart)throw new Error(`${symbol} empty chart`);
  return chart;
}
async function buildRow([symbol,name,market]){
  const chart=await yahoo(symbol),meta=chart.meta||{},rows=chartRows(chart),latest=rows.at(-1),previous=rows.at(-2);
  const stamp=Number(meta.regularMarketTime||latest?.time||0);
  if(!Number.isFinite(stamp)||stamp<=0)throw new Error(`${symbol} missing verified quote time`);
  const quoteAt=new Date(stamp*1000),session=marketDate(quoteAt,market);
  const live={open:num(meta.regularMarketOpen),high:num(meta.regularMarketDayHigh),low:num(meta.regularMarketDayLow),close:num(meta.regularMarketPrice),volume:num(meta.regularMarketVolume)};
  const useLive=[live.open,live.high,live.low,live.close].every(value=>value!==null)&&live.high>=Math.max(live.open,live.close)&&live.low<=Math.min(live.open,live.close);
  const display=useLive?live:latest;
  if(!display)throw new Error(`${symbol} missing OHLC`);
  const latestSession=latest?marketDate(new Date(latest.time*1000),market):null,prev=latestSession===session?previous?.close:latest?.close;
  const change=prev===null||prev===undefined?null:display.close-prev,percent=prev?change/prev*100:null;
  if(display.close<display.low||display.close>display.high)throw new Error(`${symbol} mixed session`);
  const windowOpen=marketOpen(market),expected=marketDate(new Date(),market),sessionConfirmed=session===expected,isOpen=windowOpen&&sessionConfirmed;
  const ageSeconds=Math.max(0,(Date.now()-quoteAt.getTime())/1000),stale=isOpen&&ageSeconds>180,unconfirmed=windowOpen&&!sessionConfirmed;
  const staleReason=stale?"盤中超過 3 分鐘未更新":unconfirmed?`尚未確認 ${expected} 交易資料；可能休市或行情尚未開出`:null;
  return {symbol,name,market,price:display.close,previous_close:prev,change,change_percent:percent,session_date:session,price_date:session,ohlc_date:session,open:display.open,high:display.high,low:display.low,close:display.close,volume:display.volume,market_at:quoteAt.toISOString(),quote_age_seconds:ageSeconds,candles:rows.slice(-70).map(x=>({...x,date:marketDate(new Date(x.time*1000),market)})),candle_count:Math.min(rows.length,70),candle_interval:"1d",data_status:stale?"stale":unconfirmed?"cached":"live",freshness_status:stale?"stale":isOpen?"live":unconfirmed?"unconfirmed":"closed",stale_reason:staleReason,market_open:isOpen,validation_status:"verified",source:"Yahoo edge same-session quote"};
}
async function hotGet(url){
  try{
    const response=await caches.default.match(new Request(url));
    return response?await response.json():null;
  }catch(error){return null}
}
async function hotPut(url,payload,ttl){
  try{
    const response=new Response(JSON.stringify(payload),{headers:{"content-type":"application/json;charset=utf-8","cache-control":`public,max-age=${ttl}`}});
...
async function refresh(env,onlyOpen=false,persistLkg=false){
  ...
  for(const config of SYMBOLS){
    ...
    try{items.push(await buildRow(config))}
    catch(error){items.push({...oldMap.get(config[0]),symbol:config[0],name:config[1],market:config[2],data_status:"cached",freshness_status:"stale",stale_reason:String(error.message||error),validation_status:"cached-last-verified"})}
  }
...
async function snapshot(env){
  ...
  try{
    const payload=await refresh(env,open,!!(!lkg));
    await hotPut(HOT_SNAPSHOT_URL,payload,open?60:600);
    return json(payload);
  }catch(error){
    if(lkg?.metadata?.schema_version===SCHEMA_VERSION)return json({...lkg,metadata:{...(lkg.metadata||{}),stale:true,stale_reason:String(error.message||error)}});
    return json({error:"live refresh unavailable",detail:String(error.message||error),version:VERSION,schema_version:SCHEMA_VERSION},503);
  }
}
async function kline(url,env){
  ...
  const cached=await hotGet(hotUrl),market=ALLOWED_SYMBOLS.get(symbol)[2],ttl=marketOpen(market)?60000:900000;
  const cachedAge=Date.now()-Date.parse(cached?.metadata?.updated_at||0);
  if(cached?.metadata?.schema_version===SCHEMA_VERSION&&Number.isFinite(cachedAge)&&cachedAge<ttl)return json(cached);
  try{
    const [range,yahooInterval]=SPECS[interval],chart=await yahoo(symbol,range,yahooInterval);
    let candles=chartRows(chart);if(interval==="4h")candles=aggregate4h(candles,market);
    if(candles.length<2)throw new Error("insufficient candles");
    const payload={metadata:{version:VERSION,schema_version:SCHEMA_VERSION,updated_at:new Date().toISOString()},symbol,interval,source:"Yahoo chart via edge worker",candles};
    await hotPut(hotUrl,payload,marketOpen(market)?60:600);
    return json(payload);
  }catch(error){
    if(cached?.metadata?.schema_version===SCHEMA_VERSION)return json({...cached,metadata:{...(cached.metadata||{}),stale:true,stale_reason:String(error.message||error)}});
    return json({error:"kline unavailable",version:VERSION,schema_version:SCHEMA_VERSION},503);
  }
}
export default{
  ...
  async scheduled(event,env,ctx){
    const minute=new Date(event.scheduledTime).getUTCMinutes(),persistLkg=minute%15===0;
    ctx.waitUntil((async()=>{
      try{
        const payload=await refresh(env,minute%15!==0,persistLkg);
        await hotPut(HOT_SNAPSHOT_URL,payload,anyMarketOpen()?60:600);
      }catch(error){console.warn("scheduled refresh failed",String(error.message||error))}
    })());
  }
};
